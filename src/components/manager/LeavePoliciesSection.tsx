...
const LeavePoliciesSection: React.FC<LeavePoliciesSectionProps> = ({ className }) => {
  ...
  const fetchPolicies = useCallback(async () => {
    try {
      setLoading(true);
      const response = await managerAPI.getLeavePolicies({
        search: searchTerm,
  ...
  const fetchStats = async () => {
    try {
      const response = await managerAPI.getLeavePolicyStats();
      if (response.success) {
        ...
        <Card className="bg-white/80 backdrop-blur-sm border-white/20 shadow-lg">
          <CardContent className="p-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <p className="text-2xl font-bold text-slate-900">{stats.inactivePolicies}</p>
                <p className="text-sm text-slate-500">Inactive Policies</p>
              </div>
            </div>
          </CardContent>
        </Card>
